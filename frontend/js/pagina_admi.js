...
// En la función initializeEvents(), agrega estos event listeners
if (btnAddProduct) {
...
function handleAddProduct() {
    ...
    const formData = {
        plant: {
            nombre_variedad: document.getElementById('nombre_variedad').value,
            especie: document.getElementById('especie').value,
            tamano_planta_cm: document.getElementById('tamano_planta_cm').value,
            color_hoja: document.getElementById('color_hoja').value,
            descripcion: document.getElementById('descripcion_planta').value
        },
        grain: {
            tamano_grano_mm: document.getElementById('tamano_grano_mm').value,
            color_grano: document.getElementById('color_grano').value,
            forma_grano: document.getElementById('forma_grano').value,
            calidad: document.getElementById('calidad').value,
            imagen_url: document.getElementById('imagen_url').value
        },
        region: {
            nombre: document.getElementById('region').value
        },
        flavor: document.getElementById('sabor').value,
        altitud_optima: document.getElementById('altitud_optima').value,
        coffee_data: {
            resistencia: document.getElementById('resistencia').value,
            densidad_plantacion: 5000, // Valor por defecto
            requerimiento_nutricion: "Medio" // Valor por defecto
        },
        time_growth: {
            Desde_anhos: 2, // Valor por defecto
            Hasta_anhos: 5  // Valor por defecto
        }
    };

    // Validar los datos (puedes agregar más validaciones según necesites)
    if (!formData.plant.nombre_variedad || !formData.grain.tamano_grano_mm || !formData.flavor) {
        showNotification('Por favor complete todos los campos requeridos', 'error');
        return;
    }

    // Enviar los datos al servidor
    fetch('http://localhost:8081/caracteristicas_cafe', {
        method: 'POST',
        headers: {
    ...
    .then(data => {
        showNotification('Nueva variedad de café agregada con éxito', 'success');
        hideModal(addProductModal);
